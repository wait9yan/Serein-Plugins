 ...
 * @LastEditors  : 9Yan
 * @LastEditTime : 2023-07-06 15:01:36
...
function bindAdd(groupID, userID, gameID) {
	...
	var bindState = serein.bindMember(userID, gameID)

	if (bindState && config.whitelist.enable && config.whitelist.autoSync) {
		const whitelistCommand = gameID.includes(' ') ? `whitelist add "${gameID}"` : `whitelist add ${gameID}`;
		serein.sendCmd(whitelistCommand);
	}

	if (bindState && config.editCard) {
...
function syncMemberToWhitelist(groupID) {
	...
	setTimeout(() => {
		let oldData = whitelist.map((item) => item.name);
		let addData = members.data.filter((item) => !oldData.includes(item.gameID));
		...
		oldData = members.data.map((item) => item.gameID);
		addData = whitelist.filter((item) => !oldData.includes(item.name));
		serein.log(addData);
		addData.forEach((item) => {
			whitelistRemoveList.push(item.name);
			let whitelistCommand = item.name.includes(' ') ? `whitelist remove "${item.name}"` : `whitelist remove ${item.name}`;
			serein.sendCmd(whitelistCommand);
		});
